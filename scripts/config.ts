...
import { DojimaConfig } from "./hermes_config";
...
DEPOSIT_CONTRACT_ADDRESS: 0x4242424242424242424242424242424242424242
    `;
  fs.writeFileSync(path.join(consts.config_path, "prysm.yaml"), prysm);
}
...
function writeGethGenesisConfig(argv: any) {
  const gethConfig = `
    ...
    `;
  fs.writeFileSync(
    path.join(consts.geth_config_path, "geth_genesis.json"),
    gethConfig
  );
...
function getChainInfo(): ChainInfo {
  const filePath = path.join(consts.config_path, "l2_chain_info.json");
...
function writeConfigs(argv: any) {
  const valJwtSecret = path.join(consts.config_path, "val_jwt.hex");
  const chainInfoFile = path.join(consts.config_path, "l2_chain_info.json");
  let baseConfig = {
    "parent-chain": {
      connection: {
        url: argv.dojimaRpcUrl,
      },
    },
    chain: {
      id: 412346,
    ...
    node: {
      ...
      "data-availability": {
        ...
        "parent-chain-node-url": argv.dojimaRpcUrl,
  ...
  if (argv.simple) {
    ...
    if (argv.simpleWithValidator) {
      ...
      fs.writeFileSync(
        path.join(consts.config_path, "validation_node_config.json"),
      ...
      if (argv.espresso) {
        ...
        const val_jwt = `0xe3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`;
        fs.writeFileSync(path.join(consts.config_path, "val_jwt.hex"), val_jwt);
      }
    }

    fs.writeFileSync(
      path.join(consts.config_path, "sequencer_config.json"),
    ...
    fs.writeFileSync(
      path.join(consts.config_path, "validator_config.json"),
      validconfJSON
    );
    ...
    fs.writeFileSync(
      path.join(consts.config_path, "unsafe_staker_config.json"),
    ...
    if (argv.espresso && argv.enableEspressoFinalityNode) {
      ...
      fs.writeFileSync(
        path.join(consts.config_path, "espresso_finality_sequencer_config.json"),
    ...
    fs.writeFileSync(
      path.join(consts.config_path, "poster_config.json"),
  ...
  const l3ChainInfoFile = path.join(consts.config_path, "l3_chain_info.json");
  ...
  fs.writeFileSync(
    path.join(consts.config_path, "l3node_config.json"),
...
function writeL2ChainConfig(argv: any) {
  ...
  fs.writeFileSync(
    path.join(consts.config_path, "l2_chain_config.json"),
    l2ChainConfigJSON
  );
}
...
function writeL3ChainConfig(argv: any) {
  ...
  fs.writeFileSync(
    path.join(consts.config_path, "l3_chain_config.json"),
    l3ChainConfigJSON
  );
}
...
function writeL2DASCommitteeConfig(argv: any) {
  ...
  fs.writeFileSync(
    path.join(consts.config_path, "l2_das_committee.json"),
...
function writeL2DASMirrorConfig(argv: any, sequencerInboxAddr: string) {
  ...
  fs.writeFileSync(
    path.join(consts.config_path, "l2_das_mirror.json"),
...
function writeL2DASKeysetConfig(argv: any) {
  ...
  fs.writeFileSync(
    path.join(consts.config_path, "l2_das_keyset.json"),
